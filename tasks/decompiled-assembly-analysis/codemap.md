...
- **`GiteaGitRepositoryTransport.cs`, `ExternalSourceRepositoryCheckoutStatus.cs` und `ExternalSourceCheckoutAttestation.cs`** — kapseln isolierte Clone-/Fetch-/Reset-/HEAD-Aktionen, den nicht-interaktiven Clean-/Dirty-/Unverified-Git-Status einschließlich Ignored-Dateien und die gebundene Re-Check-Attestation für den konkreten Checkout.
- **`ExternalSourceCheckoutMaterializationLease.cs`, `ExternalSourceRepositoryAcquisitionModels.cs` und `SourceSnapshotModels.cs`** — halten für Attestation, Cache-Publish, Workspace-Öffnung und Snapshot-Lifetime einen ownership-/lock-gebundenen Read-Lease auf Checkout-Dateien und dem Ownership-Marker; Cleanup wird bis zur letzten Materialisierungsnutzung verzögert.
- **`ExternalSourceRepositoryAcquirer.cs`, `ExternalSourceRepositoryCacheRefresh.cs`, `ExternalSourceRepositoryCacheWriter.cs` und `ExternalSourceRepositoryCacheWriterLifecycle.cs`** — erzwingen Ownership, sichere Revision/Solution und Attestation vor/nach Copy, Manifest/Inventory und Pointer-Publish; die getrennte Lifecycle-Komponente hält Rollback, Pointer/Lock-Freigabe und Source-Lease-Reihenfolge zusammen, propagiert bei stale Fehlern Last-good als sichtbares degraded Metadatum und hält CurrentChanged-Reuse getrennt vom Fallback.
- **`ExternalSourceRepositoryCacheReuse.cs`, `ExternalSourceSnapshotMaterializer.cs`, `GiteaExternalSourceProvider.cs` und `AssemblySourceSelectionOrchestrator.cs`** — binden Cache-Manifest-Hashes bzw. Workspace-Öffnung an dieselbe Checkout-Attestation, materialisieren nur verifizierte Snapshots, geben `ProviderDegraded` ohne Snapshot an die Selection weiter und lassen den statischen Decompilation-Fallback offen.
...
- **`src/AiNetLinter.FastTests/Mcp/Assemblies/GiteaGitRepositoryCheckoutStatusTests.cs`, `ExternalSourceRepositoryCheckoutAttestationTests.cs` und `ExternalSourceRepositoryCacheRefreshTests.cs`** sowie **`src/AiNetLinter.IntegrationTests/Mcp/Assemblies/ExternalSourceSnapshotMaterializerTests.cs`** — decken Clean-/Dirty-/Unverified-Gates einschließlich Ignored-/Untracked-/Malformed-Status, echte Mutation-Races an Publish- und Workspace-Grenzen, Credential-Isolation, Last-good/Degraded/Unavailable, Cleanup und CurrentChanged-Reuse mit lokalen Fakes, deterministischen Barrieren und `TestTempDirectory` ab.
