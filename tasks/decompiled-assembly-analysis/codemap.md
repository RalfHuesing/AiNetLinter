...
- **`ExternalSourceRepositoryAcquirer.cs`, `ExternalSourceRepositoryCacheRefresh.cs` und `ExternalSourceRepositoryCacheWriter.cs`** — erzwingen Ownership, sichere Revision/Solution und Attestation vor/nach Copy, Manifest/Inventory und Pointer-Publish; propagieren bei stale Fehlern Last-good als sichtbares degraded Metadatum und halten CurrentChanged-Reuse getrennt vom Fallback.
...
- **`src/AiNetLinter.FastTests/Mcp/Assemblies/GiteaGitRepositoryCheckoutStatusTests.cs`, `ExternalSourceRepositoryCheckoutAttestationTests.cs` und `ExternalSourceRepositoryCacheRefreshTests.cs`** — decken Clean-/Dirty-/Unverified-Gates einschließlich Ignored-/Untracked-/Malformed-Status, Mutation-before-publish/materialize, Credential-Isolation, Last-good/Degraded/Unavailable, Cleanup und CurrentChanged-Reuse mit lokalen Fakes und `TestTempDirectory` ab.
