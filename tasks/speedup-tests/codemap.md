...
<!-- step-013 planning: EPIC-4 Teil 1 ist auf die Skeleton-/Filterkohorte begrenzt; Scanner und
...
- **`src/AiNetLinter.TestKit/RoslynTestSolutionFactory.cs` / `src/AiNetLinter.FastTests/Platform/RoslynTestSolutionFactoryTests.cs`** — bestehender In-Memory-Builder samt Plattformverträgen; der aktuell aus dem Testhost-`AppDomain` abgeleitete Core-Referenzsatz ist vor einer testprojekt-sensitiven Filtermatrix der relevante Plattform-Einstieg. (zuletzt: planning step-013)
...
- **`src/AiNetLinter.Tests/Maps/Skeleton/SkeletonMapBuilderTests.cs`** — zwei pending Legacy-Verträge für den Pfad-/MSBuild-Adapter und den Fehlerweg bei ungültigem Pfad; bei einer neuen objektbasierten Builder-Seam gezielt mitzuführen. (zuletzt: planning step-013)
