...
- **`src/AiNetLinter.TestKit/RoslynTestSolutionFactory.cs` / `src/AiNetLinter.FastTests/Platform/RoslynTestSolutionFactoryTests.cs`** — In-Memory-Builder samt Plattformverträgen mit einmalig gecachten testframework-freien BCL-Core-Referenzen und expliziten projektspezifischen Zusatzreferenzen. (zuletzt: step-013)
- **`src/AiNetLinter.Tests/Mcp/Tools/DuplicateDetectionScannerTests.cs` / `src/AiNetLinter/Mcp/Tools/DuplicateDetection/DuplicateDetectionScanner.cs`** — naechster EPIC-4-Scanner-Schnitt: sieben Legacy-Vertraege nutzen bereits die produktive `Solution`-Seam, bauen Test-Solutions/Pfade aber noch lokal mit Temp-Dateisystem statt ueber das TestKit. (zuletzt: planning step-015)
