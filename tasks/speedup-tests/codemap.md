...
<!-- step-023: EPIC-5 implementiert, pending audit -- die verbleibenden 21 Config-/Suppression-
     Klassen liegen nach reinen FastTests-Vertraegen und isolierten Datei-IntegrationTests; TD-003
     ist im selben Configbereich mechanisch synchronisiert. -->
...
- **`src/AiNetLinter.IntegrationTests/Platform/LoadedFixture.cs` / `LoadedFixtureTests.cs` / `SolutionRootLocator.cs`** — gemeinsamer Owner mit dem statischen realen Max-2-Gate, testbaren privaten Gate-Instanzen und gemeinsamer Root-Aufloesung. (zuletzt: step-024)
- **`src/AiNetLinter.FastTests/Configuration/`, `Core/` und `Suppression/`** — zwölf reine Config-, Compound-Suppression-, CLI-Parsing- und Suppression-Policyklassen ohne Platte, MSBuild, Prozess oder echtes Repository. (zuletzt: step-023)
- **`src/AiNetLinter.IntegrationTests/Configuration/` und `Suppression/`** — neun Klassen mit echten Config-/Suppression-Dateien, Temp-Verzeichnissen oder isolierter `BaselineMini`-Mutation; ohne MSBuild-Katalogload und ohne globale Collection. (zuletzt: step-023)
- **`src/AiNetLinter.IntegrationTests/Fixtures/FixtureWorkspaces.cs`** — vorhandener `IsolatedFixtureLease`-Adapter samt `BaselineMiniFixtureWorkspace`, der den letzten Suppression-Workspace-Konsumenten ohne Legacy-Helper oder MSBuild-Load aufnimmt. (zuletzt: planning step-023)
...
- **`src/AiNetLinter/Baseline/SourceFileCatalog.cs` / `SourceFileCatalogLoader.cs`** — MSBuild-freier Catalog-Kern und interner MSBuild-Loader hinter dem kompatiblen `LoadAsync`-Adapter. (zuletzt: step-024)
- **`src/AiNetLinter.FastTests/Baseline/SourceFileCatalogPolicyTests.cs` / `src/AiNetLinter.IntegrationTests/Baseline/SourceFileCatalogAdapterTests.cs`** — fachlicher Split der Catalog-Policy- und echten Adaptervertraege. (zuletzt: step-024)
...
- **`src/AiNetLinter.Tests/Configuration/`, `Suppression/`, `Cli/IgnoreSuppressions*` und `Core/CompoundSuppression*`** — 21 noch pending Config-/Suppression-Quellen des step-023-Schnitts; 12 besitzen nur schnelle Verträge, neun reale isolierte Dateiverträge. (zuletzt: planning step-023)
