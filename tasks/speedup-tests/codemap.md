...
- **`.agents/rules/AiNetLinterRichtlinien.mdc`** — enthaelt die projektspezifischen Test-, Parallelitaets-, MCP- und Commitregeln. (zuletzt: planning)
...
- **`src/AiNetLinter.Tests/Cli/FilterCliIntegrationTests.cs`** — 18-faellige Filtermatrix gegen die komplette eigene Solution und groesster konsistenter Migrationshebel. (zuletzt: planning)
...
- **`src/AiNetLinter.Tests/Mcp/McpLiveRepositoryTests.cs`** — Dogfood-Matrix gegen das echte Repository ueber eine geteilte Serverfixture. (zuletzt: planning)
