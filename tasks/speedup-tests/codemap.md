...
- **`src/AiNetLinter.Tests/Mcp/Tools/CallTreeMermaidRendererTests.cs` / `MetricsTreeRendererTests.cs`** — letzte noch `pending`e EPIC-3-Kohorte: acht reine Unit-Tests fuer Mermaid- und ASCII-Baumformatierung, ohne Helper, MSBuild, Prozess oder Repository. (zuletzt: planning step-012)
...
- **`tasks/speedup-tests/test-migration-ledger.md`** — Inventar aller 183 Legacy-Testklassen mit Status, Legacy-Filter und neuem Abdeckungsort; 28 Checker- und fuenf Web-Parser-Zeilen stehen auf `migrated`. (zuletzt: step-011)
