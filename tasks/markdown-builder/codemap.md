...
- **`src/AiNetLinter/Output/MarkdownBuilder.cs`** — *geplant (EPIC-01)*: neuer, einziger Owner für `ColumnAlign`, `MarkdownTableBuilder` (Tabelle mit Escaping, beiden `Table`-Überladungen) und `MarkdownBuilder` (fluent: `Heading`, `BlankLine`, `Line`, `BulletList`, `CodeBlock`, `Table`, `AppendTo`, `Build`).
- **`src/AiNetLinter/Output/ViolationMarkdownFormatter.cs`** — *Prio 3 (EPIC-01)*: handlungsanweisungsorientierte Markdown-Ausgabe von `RuleViolation`-Listen, enthält die umzubauende `BuildSummaryTable` (bedingte `hasStructural`-Spalte) — der eingerückte Code-Block in `AppendViolationItem` (Z.263–268) bleibt absichtlich unberührt.
...
- **`src/AiNetLinter/Mcp/Tools/FileStructure/GetClassStructureTool.cs`** — *Prio 1 (EPIC-01)*: tabellarische Member-Übersicht (`AppendMemberRows` Z.322–351) mit bedingter `isMultiFile`-Spalte und der privaten `FormatMemberRow`, die nach Migration toter Code wird und gelöscht werden muss.
- **`src/AiNetLinter/Mcp/Tools/Analysis/GetViolationsScanner.cs`** — *Prio 2 (EPIC-01)*: `AppendSection` Z.248–261 baut Header + Separator + Tabellenzeilen + Snippet-Code-Block in einer Schleife — Reihenfolge „Zeile → Snippet" muss identisch bleiben, Migration daher in derselben `foreach` (Tabelle + `MarkdownBuilder.CodeBlock`).
...
- **`src/AiNetLinter.FastTests/Output/MarkdownBuilderTests.cs`** — *geplant (EPIC-01)*: neue Testklasse, ≥22 Fälle aus Konzept §4 (EscapeCell/Alignment/Header/Build/CodeBlock-Edge-Cases/Dokument-Mix), `[Trait("Category", "Unit")]`.
