...
- [ ] **EPIC-02: Verbleibende Callsites umstellen + HotspotSectionFormatter entfernen** — Bündelt Konzept-Schritte 7 + 8 + 9 + 10 + 11 + 12 (Hotspot-Cleanup + die sechs übrigen, seit Konzept-Stand neu hinzugekommenen oder übersehenen Callsites). Konzept-Referenz: §3 Prio 4–10, §5 Mängel-Liste, §8 Schritte 7–12. *In Arbeit → step-003* (Welle 1: `HotspotSectionFormatter` löschen + `ListRulesCommand` + `GetSymbolBodyTool`); Wellen 2 + 3 (step-004 / step-005) folgen für `Generators` (Prio 5/7/8) + `MetricsLookupFormatter` (Prio 9) + opportunistisch TD-001.
