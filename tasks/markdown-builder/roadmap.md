...
- [ ] **EPIC-02: Verbleibende Callsites umstellen + HotspotSectionFormatter entfernen** — Bündelt Konzept-Schritte 7 + 8 + 9 + 10 + 11 + 12 (Hotspot-Cleanup + die sechs übrigen, seit Konzept-Stand neu hinzugekommenen oder übersehenen Callsites). Konzept-Referenz: §3 Prio 4–10, §5 Mängel-Liste, §8 Schritte 7–12. *Welle 1 done (`step-003`, `107b2682`, `approved` — `HotspotSectionFormatter` gelöscht, `ListRulesCommand` + `GetSymbolBodyTool` umgestellt; siehe `step-003/step-review.md` für Audit-Verdict); Welle 2 in Arbeit → step-004 (drei `Generators`-Callsites in zwei Dateien: `RepoPlaybookGenerator.AppendAgentPriority` Prio 5 + `AgentRulesGenerator.AppendMetricsTable` Prio 7 + `AgentRulesGenerator.AppendCompoundSuppressions` Prio 8, mit produktiver `MarkdownBuilder.Table(MarkdownTableBuilder)`-Instanz-Überladung — obsoleted TD-002); Welle 3 in Folge → step-005 (`MetricsLookupFormatter` Prio 9 + opportunistisch TD-001).*
