...
- `src/AiNetLinter/Mcp/Assemblies/Analysis/AssemblyAnalysisResponse.cs`: letzter zentraler Wire-Trim für Assembly-Session-Routen; misst Text plus Structured gemeinsam. Die Envelope-Rekonstruktion ist aktuell auf `types`/`extensions` sowie den Composite-Root begrenzt; route-spezifische Arrays/Knoten (z. B. `fileTree`, `callSites` und Body-Ergebnisse) werden nicht fachlich zurückgezählt.
