...
- `src/AiNetLinter/Mcp/Assemblies/Analysis/AssemblyAnalysisResponse.cs`: letzter zentraler Wire-Trim für alle Assembly-Session-Routen; misst Text, Structured Content und Wire-Metadaten gemeinsam, schützt unbekannte Arrays und lehnt nicht repräsentierbare Budgets unterhalb der dokumentierten Mindestgröße recoverable ab.
- `src/AiNetLinter/Mcp/Assemblies/Analysis/AssemblyAnalysisResponseEnvelope.cs`, `AssemblyAnalysisResponseRequest.cs`: kapseln die route-spezifische Rekonstruktion für `types`/`extensions`, fileTree-Dateien/-Verzeichnisse, callSites, Body-Results, Members, Referenzen, Diagnostics, Samples und Namespaces sowie das verlustfreie Outer-/Inner-Composite-Merging.
...
- Jede finale Array-/Knotenprojektion aktualisiert ihre fachlichen Counts, Completeness-/Truncation-Gründe und Fortsetzungstoken; entfernte Composite-Sektionen bleiben als Status-/Detail-Hinweis oder über ihre verschachtelten Ergebnis-Envelopes adressierbar.
...
- Fast-Regressionen decken Budget-Konfiguration, stabile IDs, Cursor-Paging, fileTree-Counts, callSites-/Body-Results-Counts, Outer-/Inner-Composite-Status und strukturierte Bodies ab. Die Dispatcher-Regressionen prüfen nach dem finalen Trim tatsächliche Counts/Continuation bei 4096 Byte, Mindestbudget-Ablehnung, Aufrufbudget gegenüber Lease-Default, die gemeinsame Text-/Structured-Gesamtgröße sowie den Body-Detailhinweis; `topN` und Body-Provenienz bleiben regressionsgesichert. Die abschließenden Audit- und Testnachweise werden im terminalen Bericht dieses Korrekturlaufs geführt.
