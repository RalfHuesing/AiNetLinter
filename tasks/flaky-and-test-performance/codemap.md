...
- **`src/AiNetLinter.Tests/Core/Checkers/`** — 27 Klassen total, davon 7 bereits getaggt (`MaxInheritanceDepthTests`, `MaxConstructorDependenciesTests`, `MaxBoolParameterCountTests`, `MaxPublicMembersPerTypeTests`, `MaxSwitchArmsTests`, `NamespaceDirectoryMappingTests`, `NestedTypesCheckerTests`); 20 ungetaggte Klassen in 1 Mega-Batch step-011 abgeschlossen (12 Klassen M–W: `MethodParameterCountIgnoreTypePrefixesTests`, `MethodParameterCountOverrideTests`, `MiddleManCheckerTests`, `NamespaceCouplingCheckerTests`, `NamingCheckerTests`, `PhantomDependencyCheckerTests`, `SealedClassCheckerTests`, `SilentCatchAllowedTypesTests`, `SwitchDispatcherDetectorTests`, `UiFileSeparationCheckerTests`, `ValueObjectCheckerTests`, `WpfCodeBehindTests`; alle mit `[Trait("Category", "Unit")]` auf Klassen-Ebene versehen) + 8 step-010 (A–MethodParamA); **Schnitt-Annotation: 2/3 done (20 ungetaggte Klassen alle abgeschlossen), 1/3 in step-010 vorab**; `Core/Checkers/`-Ordner vollständig abgehakt (7 vorab-getaggt + 8 step-010 + 12 step-011 = 27/27) (zuletzt: step-011)
- **`src/AiNetLinter.Tests/Core/`** — 19 Klassen, alle Unit mit `[Trait("Category", "Unit")]` auf Klassen-Ebene; step-011 = 8 Klassen A–`LinterEngineCacheTests`, step-012 = 11 Klassen `LinterEngineTests`–`ViolationDescriptionTests`; **Ordner vollständig abgehakt 19/19** (zuletzt: step-012; CodeMap-Nachzug Planer step-013)
- **`src/AiNetLinter.Tests/Maps/`** + **`Maps/Skeleton/`** — 6 Klassen (`HotspotMapBuilderTests`, `SkeletonMapBuilderTests`, `SkeletonStableIdTests`, `SkeletonSyntaxWalkerTests`, `StructureMapBuilderTests`, `VocabularyMapBuilderTests`), alle Unit mit Klassen-Trait in step-012; **Ordner vollständig abgehakt 6/6**; `SkeletonStableIdTests` LF-only (TD-003-Analogon) (zuletzt: step-012; CodeMap-Nachzug Planer step-013)
- **`src/AiNetLinter.Tests/Mcp/Tools/`** — 17 Klassen, alle ungetaggt → geplant als **ein** Mega-Batch step-013 (homogen Unit: Catalog-/Mini-Fixtures, kein Subprozess); 2 LF-only (`GetServerHealthToolTests`, `ReloadConfigToolTests`); Helper `DiRegistrationMiniFixtureWorkspace` ausnehmen (Heuristik-Punkt 6) (zuletzt: step-013-Plan)
