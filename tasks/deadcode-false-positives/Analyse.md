...
| Per Reflection entdeckte Klasse | Der isolierte `ReflectionDiscoveredClass`-Test startet in `Program.Main`, enumeriert `IPlugin`-Implementierungen mit `Assembly.GetTypes()` und aktiviert `HiddenPlugin`. Der Scanner meldet die produktiv genutzte Klasse als `unreferenced`. | Auch die Beschränkung auf Typen statt Member beseitigt den Negativbeweis-Fehler nicht. |
