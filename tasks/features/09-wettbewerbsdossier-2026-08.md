...
1. `Docs/rationale.md` überarbeiten: falsche Zitat-Titel korrigieren, die zwei thematisch verfehlten Zuordnungen (DAPLab Kat. 4, Scale AI SWE Atlas) entfernen oder durch passende Quellen ersetzen, die nicht-auffindbare 70%-Zahl streichen oder durch die tatsächlich in Palomba et al. 2018 berichteten Werte (+28 % / +21 % / +300 %) ersetzen. Das im Projekt bereits vorhandene bessere Vorbild (Epic 27, empirische Kalibrierung) als Muster nehmen.
2. `README.md`-Tool-Tabelle und `ROADMAP.md` Epic 14 korrigieren (Doku-Drift-Muster aus Abschnitt 4.3).
