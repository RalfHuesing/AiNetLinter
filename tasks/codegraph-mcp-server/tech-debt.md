...
| TD-009 | `src/AiNetLinter/Mcp/McpCodeGraphServer.cs` (Konstruktor) | mittel | ~~5/5 Parameter am `MaxConstructorDependencies`-Limit, keine Reserve für die P0/P1-`McpCodeGraphServer`-Erweiterungen aus `konzept.md`.~~ **Geschlossen durch Einheit 011** (Commits `075a8a0` + `af41a6b`): `McpCodeGraphServerOptions`-`record` eingeführt, Konstruktor nimmt jetzt 1 Parameter (`McpCodeGraphServerOptions options`), künftige P0/P1-Erweiterungen wachsen additiv als Record-Property. Migration 1:1 via `From(...)`-Helper, `consoleOverride` entfernt (kein Call-Site nutzte ihn). Strukturelle A3 in `McpCodeGraphServerConstructorTests.cs` (Reflection auf Konstruktor-Signatur + `ArgumentNullException` bei `null`). |
...
| TD-014 | `src/AiNetLinter/Mcp/McpServerOptionsFactory.cs` (Footprint) | niedrig | ~~2484/2500 (16 Z. Puffer) — `ServerInstructions`-Block (+14 Z. in 003) hat die Klasse an die Grenze gebracht.~~ **Geschlossen durch Einheit 011** (Commit `4bcd5ab`): `McpServerOptionsBuilder` (54 Z., Fluent-API mit `WithServerName`/`WithServerVersion`/`WithServerInstructions`/`WithToolCollection`/`Build`) ausgelagert, Factory bleibt schlank (52 Z. mit Const + 3 Method-Bodies). Künftige P0/P1-Extensions (`--mcp-log`-State, Auto-Discovery-Hint, "lädt noch"-State) wachsen als additive `With*`-Methoden auf dem Builder, ohne die Factory selbst zu vergrößern. Strukturelle A3 in `McpServerOptionsBuilderTests.cs` (7 Unit-Tests: Default-Werte, Override-Pfade, ToolCollection-Default, Version-Fallback). |
...
| TD-016a | `src/AiNetLinter.Tests/Fixtures/{CompileErrorMini,GitImpactMini}FixtureWorkspace.cs` | niedrig | ~~Folge-Refactor aus TD-016: zwei der vier Fixture-Workspace-Klassen wurden in `6c872e4` nicht auf `FixtureWorkspaceBase` umgestellt und duplizieren weiterhin `CopyFixture`/`IsGeneratedPath`/`FindSolutionRoot`.~~ **Geschlossen durch Einheit 009** (Commits `b0c2283` + `8f0427e`): beide Klassen erben jetzt von `FixtureWorkspaceBase`, `CompileErrorMini` 71→21 Z., `GitImpactMini` 166→118 Z. (mit `ClearReadOnlyAttributes` vor `base.Dispose()` für Windows-Read-Only-Schutz auf `.git`-Objekten). Strukturelle A3-Sicherung in `TD016aRefactorTests.cs` (Reflection auf Basisklasse + Helper-Entfernung). Bonus: `CS0108` Compiler-Error fängt Re-Drift vor dem Test. |
| TD-019 | `src/AiNetLinter.Tests/Fixtures/SymbolGraphMcpFixture.cs` (parallele Test-Init) | niedrig | ~~MCP-Server-Init-Timeout bei 16 parallelen Test-Collections im ersten Volllauf von 010 (Re-Run in 1s grün, kein 010-Regress).~~ **Geschlossen durch Einheit 011** (Commit `1201840`): `McpTestClient.ConnectAsync` bekommt Retry-Loop mit exponentiellem Backoff (Default 3 Retries, 500ms/1000ms/2000ms = max 3.5s zusätzliche Wartezeit), Fixtures reichen defensivere Strategie durch (5 Retries, 1s/2s/4s/8s/16s = max 31s, im Median <5s). A3: `McpTestClientRetryTests` (3 Unit-Tests, demonstriert Retry-Verhalten mit deterministischem Failure-Pfad), `McpTestClientParallelTests` (1 Integration-Last-Test, 16 parallele `ConnectAsync`-Aufrufe). |
...
- **Vorschlag:** ~~Bei der nächsten Erweiterung an `McpCodeGraphServer` den Konstruktor auf ein Input-`record` umstellen (analog zum Vorschlag in TD-007 für `TryApplyContentChange`). Konkret: ein `internal sealed record McpCodeGraphServerOptions(SourceFileCatalog? Catalog, ILintConsole Console, int MaxLineCount, Config Config)` (oder vergleichbar). Dadurch wachsen zukünftige Konfigurations-Erweiterungen am `record` (additive Property), nicht an der Parameterliste.~~ **Erkannt im Review von Einheit 001** (Kritiker-Vorschlag in `units/001/review.md` Abschnitt "Tech-Debt-Vorschlag"). **Umgesetzt durch Einheit 011** (Commits `075a8a0` + `af41a6b`): Konstruktor nimmt jetzt 1 Parameter vom Typ `McpCodeGraphServerOptions`, künftige P0/P1-Extensions wachsen additiv am Record. `McpServerCommand.cs:36` + 64 Test-Call-Sites via `McpCodeGraphServerOptions.From(...)`-Helper migriert. A3 in `McpCodeGraphServerConstructorTests.cs`.
- **Status:** **geschlossen** durch Einheit 011 (Commits `075a8a0` + `af41a6b`).
...
- **Vorschlag:** ~~Vor der nächsten substanziellen Erweiterung an `McpServerOptionsFactory` (z. B. bei Einbau des `--mcp-log`-Flags aus P0/P1) eine Aufteilung prüfen — z. B. ein `McpServerOptionsBuilder`-Pattern (analog `McpServerCommand`-Aufteilung in `McpServerOptionsFactory` + Registrar-Klassen) oder ein Init-`record` (analog TD-009-Vorschlag für `McpCodeGraphServer`).~~ **Umgesetzt durch Einheit 011** (Commit `4bcd5ab`): `McpServerOptionsBuilder` (54 Z.) + schlanke Factory (52 Z. mit Const + 3 Method-Bodies). Künftige P0/P1-Extensions wachsen als additive `With*`-Methoden auf dem Builder. A3 in `McpServerOptionsBuilderTests.cs` (7 Unit-Tests).
- **Status:** **geschlossen** durch Einheit 011 (Commit `4bcd5ab`).
...
### TD-019 — MCP-Server-Init-Timeout bei 16 parallelen Test-Collections [Priorität: niedrig]

- **Ort:** `src/AiNetLinter.Tests/Fixtures/SymbolGraphMcpFixture.cs` (Init-Pfad via `McpTestClient.ConnectAsync`).
- **Befund:** Im ersten Volllauf von Einheit 010 (Commit `9368011` ff.) schlug genau 1 von 1178 Tests mit `TaskCanceledException` in `SymbolGraphMcpFixture.InitializeAsync` fehl. Re-Run in 1s grün, also **kein 010-Regress** — klassische Race-Condition in der `StdioClientTransport`-Startphase, wenn 16 Test-Collections parallel denselben `AiNetLinter.exe`-Subprozess starten. **Erkannt im Review von Einheit 010** (Kritiker-Vorschlag).
- **Vorschlag:** Retry-Logik in `McpTestClient.ConnectAsync` mit exponentiellem Backoff. Default-Strategie: 3 Retries, 500ms/1000ms/2000ms (Backoff-Faktor 2.0) = max ~3.5s zusätzliche Wartezeit pro `ConnectAsync`-Aufruf im Worst-Case. Fixtures reichen defensivere Strategie durch (5 Retries, 1s/2s/4s/8s/16s = max 31s, im Median <5s).
- **Status:** **geschlossen** durch Einheit 011 (Commit `1201840`):
  - `McpTestClient.ConnectAsync` bekommt Retry-Loop mit optionalem `McpTestClientRetryOptions`-Parameter (Default 3 Retries).
  - Drei Fixtures (`SymbolGraphMcpFixture`, `BaselineMcpFixture`, `McpLiveRepositoryFixture`) reichen Retry-Options explizit durch (5 Retries, 1s BaseDelay, 2.0 Factor).
  - A3: `McpTestClientRetryTests` (3 Unit-Tests: deterministischer Failure-Pfad mit bogus dir, Default-Werte, Override-Werte).
  - A3: `McpTestClientParallelTests` (1 Integration-Last-Test, 16 parallele `ConnectAsync`-Aufrufe, alle erfolgreich).
  - Beobachtung: Der 010-Flake ist unter isolierter Last nicht deterministisch reproduzierbar — der Last-Test selbst lief in 1m 39s grün, auch **ohne** Retry-Loop (rotbiegen-Verifikation). Retry-Logik ist die strukturelle Absicherung gegen den realen Flake im Volllauf.
