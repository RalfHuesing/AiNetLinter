...
| 02 `DashboardQuery` | Der historische `test_only`-Vorwurf bestätigt sich für den gemeldeten Konstruktor nicht als Fehlalarm: `DashboardService.GetDashboardAsync(DashboardQuery query, ...)` verwendet den Typ als Parameter, ruft aber keinen Konstruktor auf. Der damals fehlerhafte Handoff ist durch die Konstruktor-Auflösung und ihre Regressionstests abgedeckt. | `a345358b`, `1a392a6e` |
...
| Verify-Evidenzzähler | Bestätigte Verständlichkeitslücke behoben: `population=gate_violations+all_advisories` erläutert den Nenner von `returned=X/Y`; Dead-Code-Zähler bleiben separat. Rot-Test, FastTests und MCP-Transporttest belegen den Vertrag. | `8bcd8f4b` |
