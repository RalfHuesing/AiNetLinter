...
- `AssemblyAnalysisLease.ResolveBodyAsync`, `AssemblyDecompilationAdapter.CreateBodyResolver`, `AssemblyBodySyntax` und `GetSymbolBodyTool`: Assembly-Bodies werden ausschließlich innerhalb eines aktiven Leases on demand dekompiliert. Source-backed nutzt weiterhin Roslyn-Syntax; abstract/extern/interface/nicht verfügbare Symbole liefern explizite Hinweise. Deadline, Cancellation und `maxBodyLines` begrenzen den Resolver; interne Cachepfade werden nicht ausgegeben.
...
- Gezielt verifiziert: `GetSymbolBodyTool`/Assembly-Selection/Path-Contracts (48 FastTests), `ExternalSourceSnapshotMaterializer` (6 IntegrationTests), Literalregression (1 FastTest), Build ohne Warnungen und `git diff --check`. Die vollständigen Nicht-Stress-Gates sowie der abschließende MCP-Violations-Audit wurden auf Nutzerwunsch nicht weiter abgewartet; der nachgelagerte Orchestrator übernimmt diese Verifikation.
