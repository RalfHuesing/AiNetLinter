...
| `get_call_tree` | `targetPath` (Pflicht, absoluter vorhandener Solution- oder Assembly-Dateipfad), `symbolIdentifier` (Content-Handoff-ID), `depth?` (Default 2, hard cap 5), `format?` (`ascii` Default oder `mermaid`), `topN?` (Default 10), `direction?`, `scopeType?`, `includeGenerated?`, `includeReferences?`, `includeBcl?`, `maxResponseBytes?` | Statischer Graph mit `nodes` und `edges`. Jeder navigierbare Knoten liefert seine kanonische `handoffId` (ASCII: Knotenliste, Mermaid: Kommentar). `incoming` enthält nur echte Aufrufe, keine Override-Deklarationen. Scope, Generated-Opt-in, Tiefe, Fan-out und Budget bestimmen die sichtbaren ganzen Kanten; die Content-Marker `completeness` und `next` zeigen Begrenzungen. | ja | ja |
