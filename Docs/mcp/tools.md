...
| `get_verify_advisories` | `targetPath` (Pflicht, absoluter vorhandener `.sln`- oder `.slnx`-Pfad), `category` (Pflicht; ausschließlich `dead_code`) | Scannt die produktiven Dead-Code-Advisories der Solution bei jedem Aufruf neu, unabhängig von `verify`. Content-only, maximal 65.536 UTF-8-Bytes; nur vollständige Einträge. `candidates`, `shown` und `truncatedBy` unterscheiden Scanmenge und Ausgabe; `truncatedBy > 0` nennt die Auslassung ausdrücklich. Symbol-Handles sind direkt für `find_references`, `get_symbol_body` und `get_feature_context` nutzbar; jeden Kandidaten vor Änderungen einzeln gegenprüfen. | ja | feste, serverseitige Projektion |
...
`get_verify_advisories(targetPath, category="dead_code")` berechnet die
produktive Dead-Code-Liste bei jedem Aufruf neu und startet keinen Verify-Gate-
Lauf. Die Content-only-Antwort ist auf 65.536 UTF-8-Bytes begrenzt und enthält
nur vollständige Einträge. `candidates` zählt das Scanergebnis; `shown` zählt
die Einträge in der Antwort und `truncatedBy = candidates - shown` die
ausgelassenen. `status=partial` beschreibt einen unvollständigen Scan,
unabhängig davon, ob die Liste selbst gekürzt ist. Bei `truncatedBy > 0` nennt
der Content die Auslassung ausdrücklich; die sichtbare Liste ist dann nicht
vollständig. `symbolIdentifier` kann unverändert an `find_references`,
