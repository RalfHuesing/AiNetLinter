...
- **Metadata-only Assembly-Analyse:** `inspect_assembly` listet die öffentliche API einer exakt angegebenen absoluten lokalen `.dll`; zusätzlich sind exakte Typauswahl, Mehrfach-Memberfilter, Member-Limits sowie strukturierte Parameterdaten aus den .NET-Metadaten verfügbar. `find_assembly_extensions` findet klassische C#-Extensions; ohne Consumer-Projekt wird ihre Roslyn-Anwendbarkeit als `not_decidable` ausgewiesen. Keine der beiden Funktionen lädt oder führt die Assembly aus. Typen sind standardmäßig auf 100 Einträge, maximal 1000, und Member je Typ standardmäßig auf 100, maximal 1000, begrenzt; fehlende Abhängigkeiten werden als `partial` diagnostiziert.
