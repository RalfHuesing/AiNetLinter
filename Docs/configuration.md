...
| `MaxDirectoryChildren`                           | Metrics | Maximale Anzahl von Einträgen (Dateien + Unterordner) in einem Verzeichnis (Standard: 0 = deaktiviert). `MaxDirectoryChildrenExemptNames`: Ordnernamen, die ausgenommen werden (Standard: `["Migrations", "Generated", "wwwroot", "obj", "bin", ".git"]`).                                                                                                                                                                                                                 |
...
| `MaxPublicMembersPerType`                        | Metrics | Maximale Anzahl öffentlicher Member (Methoden, Properties, Felder, Events) pro Typ (Standard: 15). `0` = deaktiviert. `MaxPublicMembersPerTypeExemptSuffixes`: Klassenname-Suffixe, für die die Prüfung übersprungen wird (Standard: `["Extensions", "Mapper", "Constants", "Config", "ConfigOverride", "Args"]`).                                                                                                                                                                         |
...
- `-mcp-log`, `--mcp-log` (Pfad, MCP-Modus): Optionaler Pfad für das MCP-Call-Log (JSONL-Format, ein Eintrag pro Zeile). Default: deaktiviert (kein File I/O). Pfad-Auflösung: absolut → wie angegeben; relativ → relativ zum Solution-Verzeichnis. Ohne Wert (`ArgumentArity.ZeroOrOne`): Default-Pfad `<exeDir>/logs/<solutionName>/<yyyy-MM-dd>/calls.jsonl` wird automatisch konstruiert; ist keine Solution auflösbar, bricht der Server mit Fehlermeldung auf stderr und Exit 1 ab, es wird keine Log-Datei angelegt. Leere Logs werden beim Server-Shutdown automatisch gelöscht. Bei unbehandelten Exceptions in Tool-Handlern wird eine zusätzliche JSONL-Zeile mit `level=error` und den Feldern `error_type`/`error_message`/`stack_trace` (4 KB Cap) in dieselbe Datei geschrieben. Format und Felder: [Docs/agent-api.md#call-log-opt-in](agent-api.md#call-log-opt-in).
