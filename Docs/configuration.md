...
- **Metadata-only Assembly-Analyse:** `inspect_assembly` listet die öffentliche API einer exakt angegebenen absoluten lokalen `.dll`; `find_assembly_extensions` findet klassische C#-Extensions und kann ihre Roslyn-Anwendbarkeit gegen einen optionalen Consumer-Typ prüfen. Keine der beiden Funktionen lädt oder führt die Assembly aus. Ergebnisse sind standardmäßig auf 100 Einträge, maximal 1000, begrenzt; fehlende Abhängigkeiten werden als `partial` diagnostiziert.
