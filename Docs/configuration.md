...
| `AllowTryPatternOutParameters` | Global | Erlaubt `out` in `Try*`- und `Is*`-Methoden, deren Rückgabetyp in `AllowTryPatternOutParametersReturnTypes` enthalten ist (Standard: `true`). |
| `AllowTryPatternOutParametersReturnTypes` | Global | Rückgabetypen, für die `AllowTryPatternOutParameters` gilt. Einfacher Typname ohne Namespace und ohne `?` (Nullable wird automatisch entfaltet). Standard: `["bool"]` (klassisches Try-Pattern). Erweiterung auf `["bool", "string"]` deckt das Result-Message-Pattern (`string? TryXxx(...)`) ab. |
...
| `ComplexityNearMissTolerance` | Metrics | Toleranzbereich über dem Komplexitätslimit. Verstöße, deren Komplexität exakt im Bereich `(Limit, Limit + Toleranz]` liegt, werden als Near-Miss behandelt. Standard: `0` (deaktiviert). Verhalten → `ComplexityNearMissMode`. |
| `ComplexityNearMissMode` | Metrics | Verhalten bei Near-Miss-Verstößen: `"tag"` — Verstoß wird mit `[near-miss: knapp über Limit]`-Hinweis gemeldet (Standard; bisheriges Verhalten, zählt als echter Verstoß). `"suppress"` — Near-Miss-Verstöße werden vollständig unterdrückt (kein Exit-Code-Beitrag). `"warn"` — Verstoß wird mit `[near-miss: nur Warnung]`-Hinweis gemeldet. |
