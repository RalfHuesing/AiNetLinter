...
| `get_namespace_tree` | `targetPath` (Pflicht, absoluter vorhandener Solution- oder Assembly-Dateipfad), `project?` (Projektname/Substring), `namespacePrefix?` (Start-Namespace), `depth?` (1-3, Default 1), `includeTypes?` (Default true), `kind?` (class/interface/record/struct/enum/all, Default all), `maxResults?` (Default 50, Cap 200), `maxResponseBytes?` (Default 0 = kein zusätzlicher Call-Budget-Trim, Cap 65.536; negative Werte und positive Werte unter 512 sind feldgenau `INVALID_ARGUMENT`) | Hierarchischer Namespace- und Typ-Baum (3 Zoom-Stufen: Solution-Overview, Namespaces, Typ-Liste mit Datei/Zeile/Sichtbarkeit); das Budget gilt für die kombinierte UTF-8-Wire-Nutzlast aus finalem Text, `structuredContent`, Navigation und Trunkierungsfooter. Bei Budgettrunkierung werden Text und `structuredContent` aus derselben sichtbaren Teilmenge erzeugt; die finale Navigation wird vor der Antwort nochmals eingerechnet. Namespace- und Typ-Listen werden gemeinsam projiziert. `structuredContent` weist `totalCount`, `shownCount`, `truncated`, `truncatedBy` und `next` aus; Assembly-Targets beginnen mit `# Assembly Overview: <Name>` | ja | ja |
...
| `get_file_skeleton` | `targetPath` (Pflicht, absoluter vorhandener Solution- oder Assembly-Dateipfad), `filePaths` (Array von Pfaden fuer Batch in 1 Turn), `maxResponseBytes?` (Default 0, Cap 65.536; negative Werte/Überschreitung feldgenau `INVALID_ARGUMENT`) | Struktur-Skelett (Typen, Signaturen ohne Bodies, jeweils mit stabiler `id:` für `get_symbol_body`). Bei gesetztem Budget gilt die kombinierte finale UTF-8-Wire-Nutzlast des Textes inklusive Navigation und Trunkierungsfooter; es werden nur vollständige Typ-/Datei-Einheiten ausgegeben und ein expliziter Hinweis nennt ausgelassene Einheiten. Keine Kürzung mitten in Markdown-Codefences oder stabilen IDs; das Tool liefert weiterhin bewusst nur Text (additive Handoff-DTOs sind ein separater Vertrag). | ja | nein |
...
feldgenau `INVALID_ARGUMENT`. Das Legacy-Tool `find_magic_values` behält seine frühere
...
bisherigen Felder erhalten und StructuredContent ergänzt `requestedDepth`,
