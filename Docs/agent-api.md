...
| `get_file_tree` | supported | unsupported | supported | physischer Projekt- oder dekompilierter SourceRoot-Dateibestand; direkte Dateisystemroute |
...
| :--- | :--- | :--- | :--- | :---: |
| `get_file_tree` | `targetType="project"`, `targetPath` (Pflicht, absoluter Projektroot oder von `inspect_assembly` ausgewiesener `decompiledSourceRoot`), `root?` (relativ zu `targetPath`, Default `.`), `view?` (`tree` Default, `summary` oder `files`), `includeExtensions?`, `fileFilter?` und `excludePatterns?` (relative Pfad-Globs gegen `targetPath`), `maxDepth?` (0–32), `treeDepth?` (0–32, Default 2; effektive Scantiefe = `maxDepth` wenn gesetzt, sonst `treeDepth`; `0` = Root-Ebene), `maxResults?` (Default 200, 1–2.000), `sortBy?` (`path`, `size_desc`, `extension`), `includeMetadata?` (Default `true`), `includeLineCount?` (Default `false`) | Physische Projekt- oder dekompilierte SourceRoot-Dateilandkarte mit Dateipfaden, Verzeichnis-/Extension-Aggregation und `completeness`; die Route arbeitet direkt auf dem angegebenen Dateisystemroot und benötigt dafür keinen registrierten Roslyn-Projekt-Key. `summary` liefert nur Aggregation und begrenzte Top-Level-Verzeichnisaggregate (`maxResults` begrenzt auch die Verzeichnisliste), `tree` einen begrenzten Baum, `files` eine flache Trefferliste. Keine Dateiinhalte; Standardausschlüsse und Reparse-Point-Schutz bleiben aktiv. | nein | ja |
...
| `find_assembly_extensions` | `targetPath` (Pflicht, absoluter lokaler `.dll`- oder `.exe`-Pfad), `targetType?` (Default `"assembly"`; bei `.dll`/`.exe` wird Assembly ebenfalls inferiert), `receiverType?`, `extensionName?`, `namespace?`, `maxResults?` (Default 100, Cap 1000), `includeReferences?` (Default `false`; bounded Referenz-Assemblies einbeziehen) | Klassische C#-Extensions über `IMethodSymbol.IsExtensionMethod` mit strukturierten Parametern; `receiverType` filtert syntaktisch und ohne Consumer-Projektauflösung auf den ersten Parameter (`this`): unqualifiziert gegen den einfachen Typnamen, qualifiziert gegen den vollqualifizierten Namen, `global::`-Präfix wird entfernt, Vergleich ordnungserhaltend (case-sensitiv); die Consumer-basierte Unterscheidung `applicable`, `not_applicable` und `not_decidable` erfolgt anhand von Roslyn-Reduktion, Generics, Constraints und Konvertierungen. Referenzen/Sessions und `diagnosticsSummary` folgen denselben begrenzten Assembly-Grenzen wie `inspect_assembly` | nein | ja |
...
| `get_index_scope` | — | Dynamische Aufschlüsselung jeder tatsächlich vorhandenen Dateiendung in den Projektverzeichnissen (generierte Pfade und Null-Einträge entfallen); `.cs` ist als einzige Kategorie vollständig vom Symbolgraphen abgedeckt, Nicht-C#-Endungen wie `.json`, `.md`, `.props` oder `.slnx` sind sichtbar | nein | nein |
| `get_hotspots` | `scopeFilter?` (Projekt-Name oder solution-relativer Pfad), `maxResults?` (Default 50, Cap 200), `minLinePercentage?` (Default 80, Bereich 0–100), `scopeType?` (`production` Default, `tests` oder `all`) | `.cs`-Dateien des gewählten Scopes ab der angeforderten Auslastungsschwelle; `StructuredContent` enthält nur `critical`/`warning`-Dateien (kein `ok`-Eintrag pro Datei), bleibt nach absteigender Zeilenzahl und Pfad deterministisch sortiert und weist `totalHotspots`, `shownHotspots`, `truncated`, die effektiven Parameter sowie die Einträge aus | nein | ja |
...
| `get_symbol_body` | `symbolIdentifiers` (Array stabiler IDs/Namen/Dateizeilen fuer Batch in 1 Turn) **oder** `symbolIdentifier` (String-Alias fuer genau ein Symbol; bei gemischter Eingabe hat das Array Vorrang), `maxBodyLines?` (Default 80) | Markdown-Block mit Symbol-Body bzw. -Bodies, getrennt durch Divider, hart gekappt bei `maxBodyLines` mit Ellipse-Indikator; bei dekompilierten Assembly-Targets stammen verfügbare Bodies aus dem eager `WholeProjectDecompiler`-Projekt-Snapshot und den darin geladenen echten Roslyn-Dokumenten. Der direkte `SourceSymbolBodyResolver`-Pfad weist `contentMode=source` aus und löst keine zusätzliche Dekompilierung aus; C#-Defaultparameter aus VB.NET-Signaturen und fehlende Parametertyp-Referenzen im sichtbaren `partial`-Scope werden beim Matching berücksichtigt. Interface- sowie abstract-/extern-Member bleiben `bodyAvailability=unavailable` | ja | nein (Body) |
| `search_pattern` | `pattern` (Text oder Regex), `isRegex?` (Default `false` = case-insensitive Substring), `scopeType?` (`all` [Default], `production` schliesst Tests aus, `tests`), `maxResults?` (Default 50), `maxFiles?`, `contextLines?`, `maxResponseBytes?`, `scope?`, `includePatterns?`, `excludePatterns?`, `enrichCSharp?` (Default `false`) | Treffer im Dateibestand (alle Dateitypen) mit Match-Bereichen, optionalem Kontext und `completeness`; bei `isRegex=false` und Wildcards im Suchtext Hinweis auf `isRegex=true`; bei `enrichCSharp=true` zusätzlich `semantic` für sichtbare Treffer geladener C#-Dokumente | nein (Fallback) | ja |
...
`occurrences` zaehlt identische Literale in derselben Datei (Aggregation ueber `(category, value, filePath)`-Tupel); `minOccurrences` (Default 2 — Mindestvorkommen; 1 fuer auch Einzelvorkommen) filtert unterhalb der Schwelle. `byCategory*`-Felder aggregieren ueber alle Kategorie-Treffer (vor `maxResults`-Trunkierung). `truncated: true` wird in der Text-Antwort via `McpTruncation`-Meta-Zeile signalisiert, `summary.shownOccurrences < summary.total` ist die korrespondierende `StructuredContent`-Signalisierung.
...
Für genau ein Symbol ist alternativ der skalare Alias `symbolIdentifier: "M:..."` möglich;
bei gleichzeitiger Übergabe beider Formen hat das Array Vorrang.
