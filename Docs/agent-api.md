...
| `get_symbol_body` | `symbolIdentifiers` (Array stabiler IDs/Namen/Dateizeilen fuer Batch in 1 Turn) **oder** `symbolIdentifier` (String-Alias fuer genau ein Symbol; bei gemischter Eingabe hat das Array Vorrang), `maxBodyLines?` (Default 80) | Markdown-Block mit Symbol-Body bzw. -Bodies, getrennt durch Divider, hart gekappt bei `maxBodyLines` mit Ellipse-Indikator | ja | nein (Body) |
