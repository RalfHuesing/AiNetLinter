...
| :--- | :--- | :--- | :--- | :---: |
| `get_file_tree` | `root?` (relativ zu `targetPath`, Default `.`), `view?` (`tree` Default, `summary` oder `files`), `includeExtensions?`, `fileFilter?` und `excludePatterns?` (relative Pfad-Globs gegen `targetPath`), `maxDepth?` (0–32), `treeDepth?` (0–32, Default 2; effektive Scantiefe = `maxDepth` wenn gesetzt, sonst `treeDepth`; `0` = Root-Ebene), `maxResults?` (Default 200, 1–2.000), `sortBy?` (`path`, `size_desc`, `extension`), `includeMetadata?` (Default `true`), `includeLineCount?` (Default `false`) | Physische Projektdateilandkarte mit Dateipfaden, Verzeichnis-/Extension-Aggregation und `completeness`; `summary` liefert nur Aggregation und begrenzte Top-Level-Verzeichnisaggregate (`maxResults` begrenzt auch die Verzeichnisliste), `tree` einen begrenzten Baum, `files` eine flache Trefferliste. Keine Dateiinhalte; Standardausschlüsse und Reparse-Point-Schutz bleiben aktiv. | nein | ja |
...
| `get_symbol_body` | `symbolIdentifiers` (Array stabiler IDs/Namen/Dateizeilen fuer Batch in 1 Turn) **oder** `symbolIdentifier` (String-Alias fuer genau ein Symbol; bei gemischter Eingabe hat das Array Vorrang), `maxBodyLines?` (Default 80) | Markdown-Block mit Symbol-Body bzw. -Bodies, getrennt durch Divider, hart gekappt bei `maxBodyLines` mit Ellipse-Indikator; dekompilierte Assembly-Bodies berücksichtigen C#-Defaultparameter aus VB.NET-Signaturen und bleiben bei fehlenden Parametertyp-Referenzen im sichtbaren `partial`-Scope auflösbar | ja | nein (Body) |
...
Bei einer dekompilierten Assembly wird der konkrete Member gegen den
dekompilierten C#-Syntaxbaum gesucht. Enthält dieser durch VB.NET-Optionalparameter
...
der konkrete Member weiterhin nicht eindeutig gefunden wird.
