...
| `get_class_structure` | `symbol` (Pflicht: Typname, Datei:Zeile:Spalte oder DocCommentId), `sortBy?` (`lines` [Default], `kind`, `name`) | Tabellarische Übersicht über alle Member eines Typs (Kind, Name, Visibility, Start-/End-Zeile, Zeilenanzahl, Signatur) | ja | nein |
