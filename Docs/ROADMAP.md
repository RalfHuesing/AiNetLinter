...
- [x] **EPIC-09 — MCP-Call-Log-Erweiterung (Default-Pfad-Konvention + Error-Sink):** Erweitert das in EPIC-06 (B.7) eingeführte `--mcp-log` Opt-in-Feature um eine stabile Default-Pfad-Konvention, einen strukturierten Error-Sink fuer unbehandelte Tool-Handler-Exceptions und eine zentrale Wrapper-Huelle.
  ...
  - **Zentrale `ExecuteCallAsync`-Huelle:** Neuer `McpCallLog.ExecuteCallAsync(tool, args, handler)` buendelt `StartRecording` + Error-Hook + `RecordEnd` in einer einzigen async-Methode. 10 Tool-Handler in den vier `*ToolRegistrations`-Klassen (`SymbolGraph`, `FileStructure`, `Analysis`, `SymbolBody`) delegieren 1:1 an diese Huelle, sodass Call-/Error-Logging ohne per-Tool-`try/catch`-Wiederholung einheitlich greift. Im Opt-out-Pfad (kein `callLog`) wird ueber `OCE`-Filter kein Shutdown-Signal geloggt.
  - **CLI-Option-Update:** `ArgumentArity.ZeroOrOne` fuer `--mcp-log` in `CliOptionFactory` (optischer Pfad bleibt optional); Description-Text dokumentiert Default-Pfad-Konvention und Exit-1-Verhalten bei nicht aufloesbarer Solution.
  - **Tests:** 14 Tests in `McpCallLogTests` (10 alt + 4 `ExecuteCallAsync` neu), 5 Tests in `McpServerCommandCallLogTests` (1 obsoleter Test geloescht, 3 auf neue 4-Parameter-Signatur umgestellt, 4 neue Tests fuer Default-Pfad-Konstruktion inkl. `BuildDefaultLogPath`-Helper), 4 neue `RecordError`-Tests (Schema, Lock-Reihenfolge, 4-KB-Stack-Trace-Cap), alle gruen; `dotnet test`-Volllauf 1279/1279 gruen.
...
- **Opt-in Call-Log (`--mcp-log`)** — schlankes Call-Log (Zeitstempel, Tool-Name, Parameter, Ergebniszeilen, Trunkierung ja/nein, Dauer, Leermenge), Default aus. Status: **umgesetzt in EPIC-06** (B.7) — `--mcp-log <pfad>`-Flag, `McpCallLog` mit JSONL-Writer + Lock + leerer-File-Cleanup, Wrapper-Lambda in den drei `*Registrations.cs` mit `if (callLog is null)`-Fast-Path (kein Overhead bei deaktiviertem Log).
