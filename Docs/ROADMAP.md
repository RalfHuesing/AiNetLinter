...
- [x] **Regel: MaxDirectoryChildren** – Maximale Anzahl von Einträgen in einem Verzeichnis (Standard: 0 = deaktiviert). Verhindert Flat-Folder-Antipattern (zu viele Dateien auf einer Ebene); Empfehlung: 20–30 für Mittelklasse-Projekte. `MaxDirectoryChildrenExemptNames` für bekannte Ausnahmen wie `Migrations`, `Generated` und organisatorische `tasks`-Ordner.
...
- [x] **EPIC-11 — MCP-Server-Lebenszyklus:** Parent-Prozess-Watchdog mit automatischer PID-Ermittlung unter Windows über `NtQueryInformationProcess`, optionaler CLI-Option `--parent-pid <pid>`, CancellationToken-Verknüpfung und Exit-Code `0` bei Parent-Exit. Ohne explizite PID ist auf anderen Betriebssystemen keine automatische Parent-PID-Ermittlung implementiert. Fast-Tests für Erkennung/Watchdog sowie ein E2E-Test für die Prozessbeendigung sichern das Verhalten ab.
