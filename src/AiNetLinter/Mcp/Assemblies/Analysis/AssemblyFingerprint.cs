...
internal static class AssemblyFingerprintCalculator
{
    ...
    internal static bool TryCreate(string? assemblyPath, out AssemblyFingerprint? fingerprint, out AssemblySessionDiagnostic? diagnostic)
    {
        ...
        {
            diagnostic = new(AssemblyDiagnosticCodes.For(nameof(AssemblyFingerprintCalculator), nameof(AssemblyFingerprintCalculator.Canonicalize)), "Der Assembly-Pfad fehlt oder ist leer.", AssemblyDiagnosticSeverity.Error);
            return false;
        }
        ...
        {
            diagnostic = new(AssemblyDiagnosticCodes.For(nameof(AssemblyFingerprintCalculator), nameof(AssemblyFingerprintCalculator.TryCreate)), $"Assembly-Fingerprint konnte nicht berechnet werden: {ex.Message}", AssemblyDiagnosticSeverity.Error);
            return false;
        }
    }
