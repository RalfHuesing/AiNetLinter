...
## Audit-Ergebnis pro Tool (15 Tools; deckt nicht `dependency_graph`, `pattern_detect`, `safeguard` ab)
...
| `get_symbol_body` | `SOLUTION_NOT_LOADED`; echte Malfunction (`WORKSPACE_DIAGNOSTIC`) | `INVALID_ARGUMENT` (fehlendes/leeres `identifier`); `SYMBOL_NOT_FOUND`/`AMBIGUOUS_SYMBOL` (wiederverwendet) |
