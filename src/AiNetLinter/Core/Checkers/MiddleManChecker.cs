...
internal static class MiddleManChecker
{
    ...
    {
        if (!ctx.Config.Global.AvoidExcessiveMiddleMen) return;

        // Statische Klassen werden ignoriert
        if (node.Modifiers.Any(m => m.IsKind(SyntaxKind.StaticKeyword))) return;

        // Abstrakte Klassen werden ignoriert
        if (node.Modifiers.Any(m => m.IsKind(SyntaxKind.AbstractKeyword))) return;

        var className = node.Identifier.Text;

        // Suffix-Ausnahme prüfen
        if (HasExemptSuffix(className, ctx)) return;

        // Basisklassen-Ausnahme prüfen
        if (HasExemptBaseType(node, ctx)) return;

        // Alle öffentlichen/internal/protected Methoden und Properties ermitteln
